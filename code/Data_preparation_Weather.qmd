...
For this project, we retrieved data from the [NEA website](http://www.weather.gov.sg/climate-historical-daily/) for 63 weather stations distributed across Singapore from the years 2014 to 2023. After examining the station records file, we found that 41 stations ha some missing information for a few variables. Additionally, we identified stations that had no recorded data for entire months and others with excessive missing values, indicating significant gaps in data collection and recording. The detailed data cleaning process are documented down in the next few sections.
...
weather_unique <- weather_unique[!weather$Station %in% stations_to_remove, ]
```
...
```{r}
#| eval: false
write_csv(weather_unique_imputed, "weather_imputed.csv")
```
